...
static int resize(unsigned char *INbuff, unsigned char *OUTbuff, int Owidth, int Oheight, int width, int height) {
    int rx;
    int xscale;
    int x, y;
    int Cc, lastCc;
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            memcpy(output++, &input[ADDRESSE((int ) rx, (int ) y, Owidth)], sizeof(YUYV));
            lastCc = Cc;
        }
    }
    return 0;
...
int creatButt(int width, int height) {
    ...
    if (resize(YUYVbuttOrg, YUYVbutt, BUTTWIDTH, BUTTHEIGHT, width, height) < 0) {
